...
$$\text{velikostTabele} = \text{viloUrNaDanšte} \cdot \text{viloDelovnihDnište}$$
...
kakovost urnika:
$$k_1 \cdot u_1 + k_2 \cdot u_2 + . . . + k_{n{-}1} \cdot u_{n{-}1} + k_n \cdot un$$
...
zato si je smiselno podrobneje ogledati posamezne korake.
   ...
   označimo s $\text{predmet}_{\emph{i}}$.

   Naslednja vgnezdena zanka nas popelje po vseh predavalnicah. Predavalnico na
   \emph{j}-tem koraku označimo s $\text{predavalnica}_{\emph{j}}$.

   Za vse proste termine $\text{predavalnica}_{\emph{j}}$ izračunamo ceno vstavljanja 
   $\text{srečanja}_{\emph{i}}$, ki ga sestavlja $\text{predmet}_{\emph{i}}$, profesor, ki ta predmet predava,
   ...
   vstavljanja, da bomo $\text{predmet}_{\emph{i}}$ na koncu vstavili v termin z najboljšo
   ceno vstavljanja.

   Ko preverimo vse proste termine vseh predavalnic, vstavimo $\text{srečanje}_{\emph{i}}$
...
Optimiz\subsection{acijske metode}
...
Naj bo $\emph{n} = 100$ ter $\text{\emph{s}}_1$ sprememba urnika, ki nas je pripeljala v lokalni maksimum. To
spremembo (npr.~zamenjava $\text{srečanja}_{\emph{i}}$ in $\text{srečanja}_{\emph{j}}$) sedaj shranimo v
seznam tabujev. V naslednjem koraku izvedemo spremembo $\text{\emph{s}}_1$, ki se prav tako shrani v
...
se sprememba $\text{\emph{s}}_1$, ki vodi do lokalnega maksimuma, šele tedaj izbriše iz seznama
...
% začetek okolja verbatims
   ...
   Rešitev \emph{r}, ki smo jo dobili s pomočjo požrešne metode, izboljšamo z lokalnim vzpenjanjem
   ...
   Na vsakem koraku ocenimo izboljšamo rešitev \emph{r}. Če je vrednost kriterijske funkcije
